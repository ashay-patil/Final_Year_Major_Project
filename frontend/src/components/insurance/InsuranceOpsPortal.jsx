      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400">
            Manage cashless claims, HITL reviews, and insurer network.
          </p>
        </div>
        <button
      ...
      <div className="mt-6">
        {loading && activeTab !== "directory" ? (
          ...
          <>
              ...
              <div className="space-y-4">
                {pendingReviews.length === 0 ? (
                  <div className="text-center py-12 bg-white/5 border border-white/10 rounded-2xl">
                    ...
                    <p className="text-gray-400">
                      No claims pending human review.
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {pendingReviews.map((review) => (
                      <div
                        key={review.claim_id}
                        className="bg-white/5 backdrop-blur-xl border border-rose-500/30 rounded-2xl p-6 relative overflow-hidden group hover:border-rose-400 transition-colors shadow-[0_4px_20px_rgba(0,0,0,0.1)]"
                      >
                        ...
                        <p className="text-indigo-300 font-medium mb-4">
                          {review.insurance?.insurer_name || "N/A"}
                        </p>
                      ...
                      <React.Fragment key={claim.claim_id}>
                            ...
                            <span
                              className={`px-2.5 py-1 rounded-full text-xs font-semibold ${
                                claim.state === "COMPLETED"
                                  ...
                                  : claim.state === "PAYMENT_PENDING"
                                  ? "bg-amber-500/20 text-amber-400"
                                  : claim.state === "WAITING_FOR_HUMAN_APPROVAL"
                          ...
                          <td className="px-6 py-4 text-right">
                            {claim.state === "PENDING" && (
                              ...
                              >
                                <button
                                  onClick={() =>
                                    simulateResponse(claim.claim_id, "approved")
                                  }
                                ...
                                <button
                                  onClick={() =>
                                    simulateResponse(claim.claim_id, "rejected")
                                  }
                            ...
                            {claim.state === "PAYMENT_PENDING" && (
                              <div className="flex justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                                <button
                                  onClick={async () => {
                                    try {
                                      const amt = claim.bill_amount || claim.amounts?.total_billed || 150000;
                                      await apiService.insuranceMarkPayment(claim.claim_id, amt * 0.9, 'TXN-' + Date.now());
                                      fetchData();
                        ...
                        {expandedClaimId === claim.claim_id && (
                          <tr>
                            <td
                              colSpan="5"
                              className="px-6 py-6 bg-[#0f172a]/50 border-b border-white/5"
                            >
            ...
            {activeTab === "directory" && <InsurerDirectoryAdmin />}
          </>
        )}
      </div>
